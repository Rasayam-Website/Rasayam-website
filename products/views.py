...
def register_view(request):
    if request.method == 'POST':
        ...
        with transaction.atomic():
            # Split full name into first/last for Django User model
...
def save_order(request):
    ...
    if amount_in_paise < 100:
        ...
        order.delete()
        messages.error(request, "Order amount must be at least ₹1 (100 paise).")
        return redirect('cart')
    ...
    try:
        ...
        order.delete()
        # Handle auth failures (return 401) or other Razorpay API errors (return 500)
        status_code = 401 if "authentication" in str(e).lower() else 500
        messages.error(request, f"Payment gateway error: {e}")
        return render(request, 'products/cart.html', {'error': str(e)}, status=status_code)
...
def payment_verify(request):
    ...
    if request.method == "POST":
        ...
        if not all([payment_id, razorpay_order_id, signature]):
            messages.warning(request, "Your payment could not be processed due to missing information. Your selection is still safe in your bag.")
        ...
        try:
            ...
            # Return 400 for signature mismatch or other validation errors
            messages.warning(request, "Your payment could not be verified. Your selection is still safe in your bag.")
            return render(request, 'products/payment_fail.html', {
                'title': 'Transaction Unsuccessful'
            }, status=400)
